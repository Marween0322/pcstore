...
<div class="bg-gray-50 text-black/50 dark:bg-grey dark:text-dark/50">
    <div class="relative min-h-screen flex selection:bg-[#FF2D20] selection:text-white">
        ...
        <aside class="w-1/4 p-6 bg-white rounded-lg shadow-lg mr-6">
            ...
            <form action="{{ route('graphic-cards') }}" method="GET">
                <div class="mb-6">
                ...
                <div class="mb-6">
                    ...
                    <label class="block mb-2">
                        <input type="checkbox" name="brand[]" value="NVIDIA" {{ in_array('NVIDIA', (array)request('brand', [])) ? 'checked' : '' }}> NVIDIA (79)
                    </label>
                    <label class="block mb-2">
                        <input type="checkbox" name="brand[]" value="AMD" {{ in_array('AMD', (array)request('brand', [])) ? 'checked' : '' }}> AMD (81)
                    </label>
        ...
        <!-- Main Content -->
        <div class="w-3/4">
            <h1 class="text-3xl font-bold text-black dark:text-dark mb-6">Graphic Cards</h1>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($graphicCards as $graphicCard)
                    <div class="bg-white p-4 rounded-lg shadow-lg dark:bg-zinc-900 dark:ring-zinc-800">
                        <div class="relative">
                            <img src="{{ asset('storage/pictures/' . $graphicCard['image']) }}" alt="{{ $graphicCard['name'] }}" class="w-full h-32 object-cover rounded-t-lg">
                            <span class="absolute top-2 left-2 bg-yellow-300 text-yellow-900 px-2 py-1 rounded">SAVE ₱{{ number_format($graphicCard['discount'], 2) }}</span>
                        </div>
                        <div class="p-4">
                            <h3 class="text-lg font-semibold text-black dark:text-white">{{ $graphicCard['name'] }}</h3>
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Brand: {{ $graphicCard['brand'] }}</p>
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400 line-through">₱{{ number_format($graphicCard['original_price'], 2) }}</p>
                            <p class="mt-2 text-lg font-bold text-black dark:text-white">₱{{ number_format($graphicCard['price'], 2) }}</p>
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Rating: 
                                @for($i = 1; $i <= 5; $i++)
                                    @if($i <= $graphicCard['rating'])
